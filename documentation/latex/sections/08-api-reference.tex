...
This section documents the registration and email verification endpoints. All endpoints are documented using \texttt{utoipa} annotations and the OpenAPI specification is automatically generated. Types are defined in the \texttt{api-types} crate with conditional \texttt{ToSchema} derivation for OpenAPI support.

\subsection{Endpoints Overview}

\begin{itemize}[leftmargin=*,nosep]
  \item \texttt{POST /api/register} -- Create a new user account and send verification email.
  \item \texttt{POST /api/verify-email} -- Verify email address using token from email.
...
\subsubsection*{Validation Error Codes}
...
\begin{itemize}[leftmargin=*,nosep]
  \item \texttt{requests.rs}: \texttt{RegistrationRequest}, \texttt{EmailVerificationRequest}
  \item \texttt{responses.rs}: \texttt{RegisterError}, \texttt{RegisterErrorResponse}, \texttt{VerifyEmailError}
